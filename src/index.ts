...
export default class Client {
  ...
  #keystore: SessionKeystore<
    'keychainKey' | 'credentials' | 'username' | 'displayName'
  >
  ...
  constructor(options: ClientOptions = {}) {
    ...
    this.#handleNodeAuth = (res: AxiosResponse) => {
      ...
      // Parse auth claims
      const { 1: payload } = this.#token.split('.')
      try {
    ...
    if (this.#keystore.get('keychainKey')) {
      try {
        this._hydrateKeychain().catch(() => {
          this.lock()
          options.onLocked?.call(this)
        })
      } catch (e) {
        this.lock()
        options.onLocked?.call(this)
  ...
  public async signup(username: string, password: string, displayName: string) {
    const signupParams: SignupParameters = {
      displayName,
      ...(await createSignupEntities(username, password))
    }
  ...
  public get identity(): Identity | null {
    if (!this.#authClaims || !this.#keychain) {
      return null
    }
    const username = this.#keystore.get('username')
    const displayName = this.#keystore.get('displayName')
