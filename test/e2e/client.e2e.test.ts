...
import { TwoFactorStatus, CookieNames, Plans } from '@chiffre/api'
import { EventTypes } from '@chiffre/api/dist/db/models/business/Events'

let ctx: TestContext | null = null

function generateTwoFactorToken(secret: string) {
...
test('2FA - Start enabling, but cancel', async () => {
  expect(ctx!.client.settings.twoFactor.status).toEqual(
